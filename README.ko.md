...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#네-가지-출력-하나의-흑백-그림책-논리)
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 010 예시 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 010 예시 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 010 예시 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 010 예시 8"></td>
  </tr>
</table>

## 추가 3:4 상하 구성 예시

위 16:9 예시와 같은 네 개의 독립 소스를 완전한 3:4 상하 캔버스로 다시 생성했습니다. 위쪽은 현실 사진을 유지하고 아래쪽은 Panel 010의 원본 프롬프트에 따라 재구성합니다. 영어 문구는 각 사진의 내용·분위기·함의에서만 생성했으며 모든 결과를 개별 검수했습니다.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 010 추가 상하 예시 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 010 추가 상하 예시 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 010 추가 상하 예시 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 010 추가 상하 예시 4"></td>
  </tr>
</table>
...
> 거친 검정 실루엣 · 내부의 흰 특징 · 건식 재료 종이결 · 최소 환경 기호 · 그림책 편집 소문자

XXD Panel 010는 Codex와 호환 에이전트를 위한 이미지 생성 Skill입니다. 사진의 정체성, 윤곽, 자세, 동작과 관계를 보존하고 주제 또는 핵심 상호작용을 둥글고 도톰하며 조금 서툰 하나의 거친 검정 실루엣 단위로 압축합니다. 검정 내부에서 원본 고유의 흰 특징 하나를 도려내 적은 정보로도 즉시 알아보게 합니다.
...
**Panel 010**은 원본 사진의 핵심을 보존하면서 다음과 같은 시각 방향을 원하는 작업에 적합합니다: **사진을 깨끗한 흰 종이 위 거친 검정 건식 재료 그림책 실루엣으로**. 먼저 용도와 원하는 결과를 확인한 뒤 구성, 크기, 텍스트와 전달 형식을 결정합니다.
...
> XXD Panel 010으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
## 빠른 판단: XXD Panel 010이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진을 깨끗한 흰 종이 위 거친 검정 건식 재료 그림책 실루엣으로 |
| **한눈에 알아볼 특징** | 거친 검정 실루엣 · 내부의 흰 특징 · 건식 재료 종이결 · 최소 환경 기호 · 그림책 편집 소문자 |
...
```text
원본 사실 고정 → 둥글고 도톰한 하나의 상호작용 단위로 압축 → 거친 검정 건식 재료로 반복 도포 → 검정 내부에서 결정적 흰 특징 하나를 도려냄 → 하나의 자세／동작／거리／관계가 서사를 담당 → 최소 환경 기호와 그림책 편집 소문자로 마감
```

무관한 사진으로 바꿔도 실루엣, 내부 흰 특징, 자세, 상호작용, 환경 기호와 문구가 거의 그대로 성립한다면 010가 아닙니다.

## 완성작에서 바로 알아볼 수 있는 스타일 특징

- **실루엣 단위 하나:** 최소 세 가지 원본 고유 단서로 정체성, 윤곽, 자세, 동작, 기능과 관계를 보존합니다.
- **둥글고 도톰한 형태:** 머리, 주 윤곽, 기억점을 필요할 때만 키우고 핵심 상호작용을 한 단위로 봅니다.
- **절대적인 흑백:** 거의 흰 종이와 짙은 검정 건식 재료만 사용하며 색, 포인트 색, 넓은 회색과 회색 그라데이션은 없습니다.
- **검정은 거칠게:** 힘센 크레용, 목탄, 건식 문지름에 종이결, 단절, 압력과 털 같은 가장자리를 남깁니다.
- **흰 특징 하나:** 얼굴, 창, 입구, 동물 얼굴, 사물 핵심을 검정 내부에서 도려냅니다.
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 010의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-010 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-010 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-010 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-010 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-010 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-010 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 010 적용, 공통 설정은 한 번만 확정
/xxd-panel-010 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 `~/.claude/skills/xxd-panel-010`에 연결할 수 있습니다. 설치 후 에이전트 세션을 다시 시작하세요.
...
- [Skill 워크플로](SKILL.md)
- [중문 런타임 어댑터](references/xxd-panel-010-prompt.zh-CN.md)
- [영문 런타임 어댑터](references/xxd-panel-010-prompt.en.md)
