...
The repository reserves [`assets/examples/`](assets/examples/) for future work. Only finished 010 artwork verified by the project owner will be added; no post or image from another style is used as a placeholder.

Future samples will demonstrate 010's adaptability. Their subjects, metaphors, palette, copy, and canvas ratios will never become generation references or defaults.

## Four outputs, one black-and-white picture-book logic
...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 010 rough black dry-media picture-book silhouette below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 010 rough black dry-media picture-book silhouette right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `010.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
- **Independent set:** every device references only the source photograph and may explore different silhouette placement, white feature cutout, dry-media rhythm, setting marks, and type relation.
...
## Copy must belong to the black-and-white picture-book composition
...
The image model creates the source-bound rough-black silhouette, decisive white feature cutout, credible dry-media paper evidence, sparse setting marks, one relational story, and picture-book editorial type. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
...
- [Chinese full prompt](references/xxd-panel-010-prompt.zh-CN.md)
- [English full prompt](references/xxd-panel-010-prompt.en.md)
