...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#four-outputs-one-black-and-white-picture-book-logic)
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 010 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 010 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 010 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 010 Sample 8"></td>
  </tr>
</table>

## Additional 3:4 Top–Bottom Samples

These four works reuse the same four independent sources as the 16:9 set above, now regenerated as complete 3:4 top–bottom canvases. The reality photograph stays above; the lower design follows Panel 010's original brief. English copy is generated only from each photograph's content, mood, or implication, and every result was individually reviewed.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 010 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 010 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 010 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 010 additional top-bottom sample 4"></td>
  </tr>
</table>
...
**Panel 010** is suited to work that must retain the source's essential content while pursuing this visual direction: **Compress a photograph into a rough black dry-media picture-book silhouette on clean white paper**. It begins with intended use and desired outcome; layout, size, text, and delivery format follow from those needs.
...
> Use XXD Panel 010 on this image and recommend the best layout and size first.
...
010 reverses that logic:

```text
lock source facts → compress into one rounded stocky interaction unit → build it with rough black dry media → cut one decisive white feature from inside the black → let one pose / action / distance / relation carry the story → finish with sparse setting marks and picture-book editorial type
```
...
- **Absolute black and white:** near-white paper and concentrated black dry media only—no colour, coloured accent, broad grey, or grey gradient.
- **Black must remain rough:** short forceful crayon, charcoal, or dry-rub marks retain paper grain, breaks, pressure, and hairy edges.
- **One white feature cutout:** a face, window, opening, muzzle, or object core inside the black decisively carries recognition.
...
```text
/xxd-panel-010 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-010 photo.jpg --prefs last
/xxd-panel-010 photo.jpg --prefs edit
/xxd-panel-010 photo.jpg --prefs new
```
...
/xxd-panel-010 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-010 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-010 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-010 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-010 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-010 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-010 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-010 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-010 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-010 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-010 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 010 to a whole directory; shared settings resolve once
/xxd-panel-010 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-010 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
```bash
npx skills add https://github.com/nevertoday/xxd-panel-010 --skill xxd-panel-010
```
...
Claude Code users may link the same directory to `~/.claude/skills/xxd-panel-010`. Restart the agent session after installation.
...
- [Chinese runtime adapter](references/xxd-panel-010-prompt.zh-CN.md)
- [English runtime adapter](references/xxd-panel-010-prompt.en.md)
...
**Keep black rough and white clean; one action and one field of silence can hold the whole relation.**
